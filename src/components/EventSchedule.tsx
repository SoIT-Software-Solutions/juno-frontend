  ...
  return (
    <section className="py-32 px-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 lg:gap-20">
        <div className="bg-[var(--bg-glass-black)] backdrop-blur-xl p-12 rounded-[2.5rem] text-center">
          ...
          <div className="space-y-6">
            {day1Events.map((event) => (
              <Link
                key={event.id}
                to={`/events/${event.id}`}
                className="block text-xl font-bold tracking-widest text-white/70 hover:text-yellow-500 transition-colors"
              >
                {event.name}
              </Link>
            ))}
